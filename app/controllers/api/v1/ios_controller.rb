    ...
    module V1
        class IosController < BaseController
            ...
            def login
                ...
                render :status=>200, :json=>{:success=>"1", :message=>"Success", :url=>"login", :user=>{id: @@current_user.id.to_s, name: @@current_user.name, picture: @@current_user.profile_picture.url(:thumb), phone:@@current_user.mobile_no, gender:@@current_user.gender, birth_date:@@current_user.date_of_birth.strftime("%e %B %Y").upcase, age_group:@@current_user.age_group.name, notify_new_events: @@current_user.notify_new_events.to_s}, :token=>userToken}
            end
